...
{
    public class LinearMinStatistic : TimerStatisticComputer
    {
        public override string StatisticName
        {
            get { return base.StatisticName+".lin_ms.min"; }
        }
        ...
        public override Tuple<string, object> Compute(AbstractProbe probe)
        {
            try
            {
                ...
                return new Tuple<string, object>(probe.Name + "." + StatisticName, TimeUnit.ToMilliseconds(probeValue.Item2.Min(p => p.Item2)));
            }
            catch (Exception e)
            {
